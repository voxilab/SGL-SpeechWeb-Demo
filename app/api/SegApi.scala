...
case class SegApi(spkPublicSegFile: String, mediaFileApi: MediaFileApi) {
  ...
  def getSegments(mediaFile: MediaFile, enc: String = "ISO-8859-1"): Try[List[Segment]] = getSegments(new File(mediaFileApi.getMediaDir(mediaFile), spkPublicSegFile), enc)
  ...
  def getSegmentsFromLines(lines: TraversableOnce[String]): List[Segment] = {
    lines.flatMap { l ⇒
    ...
    case Array(show, "1", start, duration, gender, chan, _, spkId) ⇒
      Some(
        (Speaker(spkId, chan, gender match {
          ...
          case "F" ⇒ Some(Female)
          case _   ⇒ None
        }), Segment(parseFloatOption(start).getOrElse(0),
