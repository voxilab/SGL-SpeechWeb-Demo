...
object AudioFiles extends Table[(Option[Int], String, String)]("audio_files") {
  ...
  val autoInc = name ~ status returning id into { 
    case ((name, status), id) => AudioFile(Some(id), name, status match {
  ...
  def findById(id: Int)(implicit session: scala.slick.session.Session): Try[AudioFile] = {
    ...
    query.firstOption.
      map(t => new AudioFile(Some(t._1), t._2)).
      asTry(badArg("AudioFile not found."))
  }
}
